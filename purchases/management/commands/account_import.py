            ...
            for row in reader:
                try:
                    ...
                    if row[5]:
                        ...
                        self.stdout.write(self.style.ERROR('No match for "%s"' % row[2]))
                        continue
                    if created:
                        self.stdout.write(self.style.NOTICE('Created "%s".' % object.account_title))
                        added_rows += 1
                    else:
                        self.stdout.write(self.style.NOTICE('Updated "%s".' % object.account_title))
                except:
                    self.stdout.write(self.style.ERROR('Unable to create/update object "%s".' % row[0]))
                    skipped_rows += 1
