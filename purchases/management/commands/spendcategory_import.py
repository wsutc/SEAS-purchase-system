        ...
        with open(path, encoding='utf8') as f:
            ...
            for row in reader:
                try:
                    ...
                    row_count += 1
                    if created:
                        self.stdout.write(self.style.NOTICE('Created "%s".' % object.description))
                        added_rows += 1
                    else:
                        self.stdout.write(self.style.NOTICE('Updated "%s".' % object.name))
                except:
                    self.stdout.write(self.style.ERROR('Unable to create/update object "%s".' % row[1]))
                    skipped_rows += 1
