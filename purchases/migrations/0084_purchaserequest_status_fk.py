...
def fill_status_fk(apps, schema_editor):
    ...
    for request in qs:
        ...
        print("New name: {}".format(new_status_name))
        request.status_fk, _ = Status.objects.get_or_create(name=new_status_name)
        print("New status[{}]: {}[{}]".format(request.number, request.status_fk.name, request.status_fk.rank))
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunSQL("ALTER TABLE `seas_purchasing`.`purchases_purchaserequest` DROP COLUMN `status_fk_id`;"),
        migrations.AddField(
            model_name='purchaserequest',
            name='status_fk',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.PROTECT, to='purchases.status'),
        ),
