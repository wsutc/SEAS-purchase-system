    ...
    for account in accounts:
        if not account.slug:
            if account.program_workday:
                ...
                return f"{account.grant} ({account.account_title})"
            elif account.gift:
                return f"{account.gift} ({account.account_title})"
            elif account.account_title:
                return account.account_title
            else:
                account.description

    _ = Accounts.objects.bulk_update(accounts, ["slug"], 100)
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunSQL(
            "ALTER TABLE `seas_purchasing`.`purchases_accounts` DROP COLUMN `slug`;"
        ),
