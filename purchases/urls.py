...
urlpatterns = [
    ...
    path(
        "purchase-request/", RedirectView.as_view(pattern_name="home", permanent=False)
    ),
    path(
        "purchase-request/open/", OpenPurchaseRequestListView.as_view(), name="open_pr"
    ),
    path("new-vendor", VendorCreateView.as_view(), name="add_vendor"),
    path("all-vendors", VendorListView.as_view(), name="all_vendors"),
    path(
        "vendor/<int:pk>-<str:slug>/", VendorDetailView.as_view(), name="vendor_detail"
    ),
    path(
        "vendor/<int:pk>-<str:slug>/update",
        VendorUpdateView.as_view(),
        name="update_vendor",
    ),
    path(
        "vendor/<int:pk>-<str:slug>/delete",
        VendorDeleteView.as_view(),
        name="delete_vendor",
    ),
    path(
        "vendor/orders/new/", VendorOrderCreateView.as_view(), name="vendororder_create"
    ),
    path(
        "vendor/orders/<int:pk>-<str:slug>/",
        VendorOrderDetailView.as_view(),
        name="vendororder_detail",
    ),
    path("vendor/orders/", VendorOrderListView.as_view(), name="vendororder_list"),
    path(
        "vendor/orders/current/",
        VendorOrderCurrentListView.as_view(),
        name="vendororder_current_list",
    ),
    path(
        "vendor/orders/<int:pk>-<str:slug>/attach-purchase-request",
        VendorOrderAddApprovedPR.as_view(),
        name="vendororder_add_pr",
    ),
    ...
    path(
        "trackers/<slug:pk>/delete/", TrackerDeleteView.as_view(), name="tracker_delete"
    ),
    ...
    path(
        "account-balances/<int:pk>/",
        BalancesDetailView.as_view(),
        name="balances_detail",
    ),
    path("account/<int:pk>/update/", update_balance, name="update_balance"),
    path(
        "all-requisitioners", RequisitionerListView.as_view(), name="all_requisitioners"
    ),
    path(
        "requisitioner/new", RequisitionerCreateView.as_view(), name="new_requisitioner"
    ),
    path(
        "requisitioner/<int:pk>-<slug:slug>/",
        RequisitionerDetailView.as_view(),
        name="requisitioner_detail",
    ),
    path(
        "simple-products/copy/<int:pk>",
        SimpleProductCopyView.as_view(),
        name="copy_simpleproduct",
    ),
    path(
        "simple-products/<purchaserequest>/",
        SimpleProductPRListView.as_view(),
        name="pr_filtered_simpleproducts",
    ),
