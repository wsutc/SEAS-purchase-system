...
class Item(models.Model):
    ...
    accessory = models.ManyToManyField(
        "self", through="EquipmentAccessory", through_fields=("item", "accessory")
    )
    ...
    purchase_request = models.ForeignKey(
        PurchaseRequest, on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    item_type = models.CharField(
        "Choose One", choices=ITEM_TYPE, default="equipment", max_length=30
    )
...
class EquipmentAccessory(models.Model):
    ...
    accessory = models.ForeignKey(
        Item, related_name="item_accessory", on_delete=models.CASCADE
    )
